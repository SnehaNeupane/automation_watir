...
if view_users == 'set'
		...
		if browser.get_browser_title == "Users | CloudFactory"
			browser.Puts "Manage user check box is checked , Production manager cannot view user profile Passed /' \e[0;31m'Failed'\e[0m' need clarification"
		else 
...
if create_new_skills == 'set'
		...
		if flag
			browser.Puts 'view_skills check box is checked in and Skill is displayed on the screen , Passed'
				Assertions.assertequal(true,true, )
			...
			if flag
				browser.Puts "create_new_skills is clear and User #{prod_usr_name} is able to find create skill button Failed"
					Assertions.assertequal(true,false, 'view_skills check box is checked in and Skill is displayed on the screen  Passed')
			else
