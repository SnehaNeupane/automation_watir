...
# Pre-info:   for now view user's profile, list all users shall not be worked , requirement not clear 
						...
						#in reports http://cloudfactory.qa/admin/reports page , choose account which has a line  or else the script will not handle the error . 
						#http://cloudfactory.qa/admin/lines/249/transactions This page should have 	 	id 463 , or else need to chaange the script . line no approx :546	
						#please change line :555 the link portion as per the data available.
						# admin2@cf.com named user much be present so as to click on it . 
						# line no 1449, line no 624, line no  795, 
						# worker id should be 2336 , line no 680, line no 1214, line no 1240.
...
browser = Browser.new(url)
	...
	browser.checkbox_action(0,'56',view_users)
	browser.checkbox_action(0,'57',create_users)# Needs more detailed information regarding this . will sscript later . 
	browser.checkbox_action(0,'58',edit_users)# Needs more detail3d infomation regarding this . will script later . 
	browser.checkbox_action(0,'59',delete_users)
	browser.checkbox_action(0,'61',view_accounts_all)
	browser.checkbox_action(0,'62',view_accounts_assigned)
	browser.checkbox_action(0,'63',create_accounts)
	browser.checkbox_action(0,'64',edit_accounts_all)
	browser.checkbox_action(0,'65',edit_accounts_assigned)
	browser.checkbox_action(0,'66',delete_accounts_all)
	browser.checkbox_action(0,'67',delete_accounts_assigned)
	browser.checkbox_action(0,'68',associate_users_all)
	browser.checkbox_action(0,'69',associate_users_assigned)
	browser.checkbox_action(0,'71',manage_roles_and_permissions)
	browser.checkbox_action(0,'73',view_workers)
	browser.checkbox_action(0,'74',edit_workers)
	browser.checkbox_action(0,'75',assign_edit_worker_skills)
	browser.checkbox_action(0,'77',view_skills)
	browser.checkbox_action(0,'78',create_new_skills)
	browser.checkbox_action(0,'79',edit_skills)
	browser.checkbox_action(0,'80',upload_workers_or_modify_skills_in_mass)
	browser.checkbox_action(0,'82',access_non_assigned)
	browser.checkbox_action(0,'83',view_lines)
	browser.checkbox_action(0,'84',create_lines)
	browser.checkbox_action(0,'85',edit_settings)
	browser.checkbox_action(0,'86',delete_lines)
	browser.checkbox_action(0,'87',view_overview)
	browser.checkbox_action(0,'88',view_gold_standards)
	browser.checkbox_action(0,'89',create_gold_standards)
	browser.checkbox_action(0,'90',view_production_runs)
	browser.checkbox_action(0,'91',create_production_runs)
	browser.checkbox_action(0,'92',delete_production_runs)
	browser.checkbox_action(0,'93',view_production_run_details)
	browser.checkbox_action(0,'94',view_production_run_results)
	browser.checkbox_action(0,'96',view_reports_all)
	browser.checkbox_action(0,'97',view_reports_assigned)
...
if view_users == 'set'
		...
		if flag
			browser.Puts "User link is displayed on the side bar when the check box for manage user is checked in, '\e[0;31m' Passed' \e[0m'"
		else
		...
		if browser.get_browser_title == "Users | CloudFactory"
			browser.Puts "Manage user check box is checked , Production manager can view user profile Passed "
		else 
		...
		browser = Browser.new(url)
			...
			if flag == false
				...
				browser.Puts "view_user check box is not checked in   and User #{prod_usr_name} is  able to view Users link/button  Failed"
			...
			browser = Browser.new(url)
				...
				browser.find_link_by_text(usr)
					...
					if flag
						...
						browser.Puts "edit_users check box is  not checked  and user #{prod_usr_name} is  able to click on #{usr} Passed"
						Assertions.assertequal(true,true,"edit_users check box is  not checked  and user #{prod_usr_name} is  able to click on #{usr} Passed")
					end
	end
end
 			 browser.close
...
if manage_roles_and_permissions == 'clear'
 ...
 if view_workers == 'set'
		...
		# if flag 
		# 	browser.Puts 'Manage worker check box is checked in and worker can view Create worker button'
		# else
		# 	browser.Puts 'Manage worker check box is checked in and create worker button is not visible'
		# end
...
if upload_workers_or_modify_skills_in_mass == 'set'
	browser = Browser.new(url)
		...
		flag = browser.find_link_by_href('/admin/skills').exists?
			...
			browser.press_enter
			sleep 1
				flag = browser.check_table_div_index(6,'upload_csv').exists?
					if flag
...
if edit_accounts_all == 'set' && view_accounts_all == 'set'
			#username = env_setting['username_cf']
		browser = Browser.new(url)
			browser.fill_input_text_by_name('user[email]', prod_usr_name )
			browser.fill_input_text_by_name('user[password]', prod_usr_pass)
...
elsif edit_accounts_all == 'clear' && view_accounts_all == 'set'
		username = env_setting['username_cf']
		browser = Browser.new(url)
			browser.fill_input_text_by_name('user[email]', prod_usr_name )
			browser.fill_input_text_by_name('user[password]', prod_usr_pass)
		...
		browser = Browser.new(url)
			...
			table_value = browser.get_value_table_withindexandid(1,1)
				...
				if view_accounts_all == 'set'
						...
						Assertions.assertequal(true,true,"view_accounts_assigned check box is checked, and user is displayed under the #{account_not_exist} , Passed")
						end
				else 
 		...
 		if view_lines == 'set'
		    ...
		    browser.select_item_list_by_name_under_frame_action("/admin/job_types/#{station_id}",'algo_set',algorithm)
				...
				sleep 1
						...
						flag = browser.get_paragraph(10)
						if flag == 'Min Payout (in $)'
	...
	if view_lines == 'set'
 				...
 				browser = Browser.new(url)
					...
					flag = browser.find_link_by_href("/admin/lines/#{production_lines_id}/overview").exists? 
						if flag
							browser.Puts "view_overview is clear and #{prod_usr_name} can click on overview Failed"
							Assertions.assertequal(true,false,"view_overview is clear and #{prod_usr_name} can click on overview Failed")
						else
							browser.Puts "view_overview is clear and #{prod_usr_name} can click on overview Passed"
							Assertions.assertequal(true,true,"view_overview is clear and #{prod_usr_name} can click on overview Passed")
						end
		else
		browser.Puts 'view_lines check box is not checked in , no further testing for edit_settings for production lines'
		browser.close
...
if create_gold_standards == 'set'
	if view_lines == 'set'
 		    ...
 		    browser.click_link_by_text('1201')
 		    browser.windows_use_last
 		    browser.div_click(0,'header header_tmp')	    #browser.click_link_by_xpath('/html/body/section/div[2]/div/div[3]/div[2]/div/div[2]/a[1]') #Clicks on 1st Job
