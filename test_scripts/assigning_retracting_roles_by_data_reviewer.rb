...
#########################Fetching Data########################
env_setting = Util.data_from_rbac['assigning_and_retracting_roles_by_production_managers'] 
...
if view_users == 'set'
		...
		if flag 
			browser.Puts " view_user check box is not checked in   and User #{prod_usr_name} is not able to view Users link/button  '\e[0;31m'Passed'\e[0m'"
	...
	browser.button_click_name('commit')	
		...
		if flag
			browser.Puts "Manage Roles check box is not checked in and Roles is displayed  , Failed"
				Assertions.assertequal(true,true,"Manage Roles check box is not checked in and Roles is displayed  Failed")
 ...
 if view_workers == 'set'
   	...
   	if flag
		...
		if flag
			...
			Assertions.assertequal(true,false, 'View worker check box is not  checked in and worker is not displayed on the screen Passed')
		end
			browser.close
end
...
if edit_workers == 'set'
	if view_workers == 'clear'
		...
		flag = browser.find_link_by_href('/admin/workers/new').exists? # finds the existence of create worker button
			...
			Assertions.assertequal(true,false,'Edit worker check box is checked in and worker can view Create worker button Passed')
			else
	...
	if view_workers == 'clear'
		...
		flag = browser.find_link_by_href('/admin/workers/new').exists? # finds the existence of create worker button
			if flag 
				...
				Assertions.assertequal(true,false,browser.Puts 'Edit worker check box is not checked in and create worker button is not visible Passed')
   	...
   	flag = browser.find_link_by_xpath('/html/body/section/div[2]/div/div[1]/div[2]/div/a').exists? #for disable button
		if flag
			...
			Assertions.assertequal(true,true,"User #{prod_usr_name} is not able to find disable the worker button Failed")
		else
		end
		browser.close
...
if create_new_skills == 'set'
		...
		if flag
			...
			Assertions.assertequal(true,false,"create_new_skills is clear and User #{prod_usr_name} is not able to find create skill button Passed")
...
sleep 1
				...
				browser.click_link_by_text(add_skill)# verifies a user is able is click on the  skilled just created by the user .
					...
					flag = browser.find_link_by_href('/admin/skills/13/download_csv').exists?
					if flag
...
if upload_workers_or_modify_skills_in_mass == 'set'
	...
	flag = browser.find_link_by_href('/admin/skills').exists?
		...
		browser.click_link_by_href('/admin/skills') # clicks on Skill link (button)
			...
			flag = browser.find_link_by_href('/admin/skills/13/upload_csv').exists?
				if flag
...
if view_reports_all == 'set'
		...
		if flag 
			...
			Assertions.assertequal(true,true,"view_reports_all check box is not checked in , Reports button is displayed Failed")
		else
...
if view_accounts_assigned == 'set'
		...
		if flag
			...
			#browser.Puts table_value(0)
				if table_value == 'No matching records found' or 'No data available in table'
...
if edit_accounts_all == 'set'
		...
		if flag
			...
			if flag 
				...
				Assertions.assertequal(true,true,"edit_accounts_all is checked in  #{prod_usr_name} is not able to  see User button Failed.")
...
browser.close
		...
		if flag
			browser.Puts "view_accounts_assigned check box is checked, and Accounts link is displayed , Passed "
			Assertions.assertequal(true,false,"view_accounts_assigned check box is checked, and Accounts link is displayed Passed ")
			browser.click_link_by_href('/admin/accounts')
			browser.index_fill_input_text(2,account_exist) # account_exist should be the one where the username_cf  belongs to this account 
			browser.press_enter
			browser.click_link_by_text(account_exist)
			...
			table_value = browser.get_value_table_withindexandid(1,1)
				if table_value == prod_usr_name 
					...
					Assertions.assertequal(true,false,"view_accounts_assigned check box is checked, and user is not displayed under the #{account_exist}  Failed")
						...
						browser.Puts "view_accounts_assigned check box is checked, and user is not displayed under the #{account_not_exist} , Passed"
						Assertions.assertequal(true,false,"view_accounts_assigned check box is checked, and user is not displayed under the #{account_not_exist}  Passed")
		...
		if flag
			...
			if flag 
				...
				browser.Puts "associate_users_all is not checked in  #{prod_usr_name} is not able to  see User button Passed."
					Assertions.assertequal(true,true,"associate_users_all is not checked in  #{prod_usr_name} is not able to  see User")
...
if view_overview == 'set'
	if view_lines == 'set'
				...
				titlebrwser =browser.get_browser_title
					...
					if titlebrwser == "Production Lines - Content Review 2 | CloudFactory"
	...
	if view_lines == 'set'
				...
				flag = browser.find_link_by_href("/admin/lines/#{production_lines_id}/overview").exists? 
					if flag
						...
						browser.Puts "view_overview is clear and #{prod_usr_name} can click on overview '\e[0;31m'Passed'\e[0m'"
						Assertions.assertequal(true,true,"view_overview is clear and #{prod_usr_name} can click on overview '\e[0;31m'Passed'\e[0m'")
					end
	else
		browser.Puts 'view_lines check box is not checked in , no further testing for edit_settings for production lines'
		browser.close
...
if view_gold_standards == 'set'
	if view_lines == 'set'
				...
				if flag == "Production Lines - Content Review 2 | Station - Content Review 2 | CloudFactory"
...
browser.close
 ...
 if view_production_run_details == 'set'
	if view_lines == 'set'
 		    ...
 		    browser.click_link_by_href('/admin/transactions/jsz61N7Ozs/jobs"') # needs to change as per the data available in the application.
 		    ...
 		    # following commented code needs to be be debugged.
 		...
 		#     if flag == /Status: /
 		#     	browser.Puts "view_production_run_details text box is checked and #{prod_usr_name}  is able to navigate on this page Passed "
 		#     else
 		#     	browser.Puts "view_production_run_details text box is checked and #{prod_usr_name}  is not able to navigate on this page Failed"
 		#     end
 		else
		 	browser.Puts 'view_lines check box is not checked in , no further testing for edit_settings for production lines'
		 	browser.close
...
if create_gold_standards == 'set'
	if view_lines == 'set'
 		    ...
 		    browser.button_click_text('Clear')
			 		 ...
			 		 #    if flag == /Data Sources for Production/
			 		 #    	browser.Puts "create_gold_standards text box is  checked and #{prod_usr_name}  is able to make GS Via DS  Passed "
			 		 #    else
			 		 #    	browser.Puts "create_gold_standards text box is  checked and #{prod_usr_name}  is not able to make GS Via DS Failed"
			 		 #    	browser.close
			 		 #    end
 		    ...
 		    # Make GS via JT
				...
				#binding.pry
 		    # browser.click_link_by_text('Runs')
 		    # browser.click_text_field_id('transaction_date_filter')# clicks on the date 
 		    # browser.button_click_text('Clear')

 		    #browser.click_link_by_text(/completed/)
 		    ...
 		    browser.fancy_box_close('fancybox-close')
 		    browser.Puts "create_gold_standards text box is   checked and #{prod_usr_name}  is able to make GS via JT on this page '\e[0;31m'Passed'\e[0m'"
