...
env_setting = Util.data_from_datafile['new_skill_wrong_input_gold_standard']
...
(1..init).each do |count|
	...
	browser.input_text_field_frame_name('work_frame', 'output[year]', 2)
	browser.input_text_field_frame_name('work_frame', 'output[month]', 2)
	browser.input_text_field_frame_name('work_frame', 'output[last_name]', "asldkfj")
	browser.input_text_field_frame_name('work_frame', 'output[given_name]', "kasjdhf")
...
puts "#{init} GS has been submitted."
...
if flag
	puts "I still got gold standard. Test Case Passed "
else
	puts "I got real task. Test case Failed."
end
