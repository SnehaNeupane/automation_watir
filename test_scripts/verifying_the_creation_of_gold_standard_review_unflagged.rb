...
#Date:April 28 2014
#Prerequisites: file is in Automation folder . For this file please use " Please check teh file name which one to use and its complete path in csv (data) file
require 'rubygems'
require '../lib/browser'
...
env_setting= env_setting1['verifying_the_creation_of_gold_standard_review_unflagged']
...
if flag 
	...
	sleep 1
 	### 	puts data_from_table_cont_1 = browser.extract_data_table(0,1,2)
 	### 	puts data_from_table_val = browser.extract_data_table(0,1,3)
	### 	puts data_from_table_cont_2 = browser.extract_data_table(0,2,2)
 	### 	puts data_from_table2_val2 = browser.extract_data_table(0,2,3)
	### sleep 1
	if browser.extract_data_table(0,2,3).to_s == review_check1 && browser.extract_data_table(0,3,3).to_s == review_check2
...
# if row found increased 
	...
	flag = browser.find_image_src_id('preview_frame','http://cfaccusoft.s3.amazonaws.com/SliceImages/24_Apr_2013/full_7ffa4695-d1e8-45b3-bd5b-abaf7fb9e54d_Age2.jpg"').exists?
			...
			data_from_vData2 = browser.extract_data_table(2,2,1)
	sleep 3

if data_from_vData1 == review_check1 && data_from_vData2 == review_check2
